...
def remapAiAdoption(df_survey: DataFrame) -> DataFrame:
    ...
    df_survey['Sector'] = df_survey['_Sector'].map(
        sector_remapping)
    print("remap of industry sector:", df_survey['_Sector'].unique(
    ).size, "to", df_survey['Sector'].unique().size)
    ...
    df_survey['Department'] = df_survey['_Department'].map(
        department_remapping)
    print("remap of company department:", df_survey['_Department'].unique(
    ).size, "to", df_survey['Department'].unique().size)
...
def categorizeTools(tools_str):
    ...
    tool_categories = {
        ...
        'Reconnaissance faciale/biometrique': [
            ...
            'ordinateur', 'accès zone restreinte…)'
        ],
        'Language Model': [
...
def addNumericalLikertScale(df_survey: DataFrame) -> DataFrame:
    ...
    df_survey['*WillingnessForTraining'] = df_survey['WillingnessForTraining'].map({
        'Oui, certainement': 2,
        'Peut-être': 1,
